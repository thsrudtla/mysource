...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			//2. Business logic 처리(로그인 처리)
			Member member = authenticate(id, password);
			//인증성공 - 로그인 처리 : HttpSession생성 + 로그인여부 체크 속성추가.
	...
	public Member authenticate(String id, String password) throws LoginFailException{
		...
		//Member member = memberDao.selectMemberById(id); //만약 Dao가 있었다면 이렇게 했을것임.
			//select id, password, name, email from member where id=?
		Member member = null; //없는 ID
		member = new Member(id, "servlet","김회원","kim@a.com");
		
		if(member != null) {//있는 ID
			if(password.equals(member.getPassword())) {
				//인증 성공
				return member;
			}else {//ID는 맞는데 Pwd는 틀린경우 - 인증실패
